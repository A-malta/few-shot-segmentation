...
class AverageMeter:
    ...
    def __init__(self, dataset):
        ...
        self.class_ids_interest = torch.tensor(self.class_ids_interest).to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        ...
        self.intersection_buf = torch.zeros([2, self.nclass]).float().to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
        self.union_buf = torch.zeros([2, self.nclass]).float().to(torch.device("cuda" if torch.cuda.is_available() else "cpu"))
