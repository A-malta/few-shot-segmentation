...
if __name__ == '__main__':
    ...
    if 'RANK' in os.environ and 'WORLD_SIZE' in os.environ:
        ...
        model.to(device)
        model = nn.parallel.DistributedDataParallel(model, device_ids=[args.local_rank] if torch.cuda.is_available() else None, output_device=args.local_rank if torch.cuda.is_available() else None,
        ...
        model = DCAMA(args.backbone, args.feature_extractor_path, False)
        model.to(device)
        # Mock module attribute for non-DDP model to keep compatibility with train function
    ...
    for epoch in range(args.nepoch):
        if hasattr(dataloader_trn.sampler, 'set_epoch'):
